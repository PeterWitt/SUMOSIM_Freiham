...
def rebalancing_main():
    ...
    shuttle_parked = shuttle_parked_main_park + traci.parkingarea.getVehicleIDs("pasing_park1") + traci.parkingarea.getVehicleIDs("pasing_park2")
    ...
    for shuttle_name in shuttle_to_be_rebalanced:
        if not traci.vehicle.isStoppedParking(shuttle_name): 
            shuttle_edge = traci.lane.getEdgeID(traci.vehicle.getLaneID(shuttle_name) if traci.vehicle.getLaneID(shuttle_name) != '' else "parked")
            try:
...
def rebalancing_helper():
    ...
    empty_parkingspace_pasing1 = int(traci.simulation.getParameter("pasing_park1", "parkingArea.capacity")) - (num_vehicle_pasing1 + len(shuttle_rebalanced["pasing1"]))
    empty_parkingspace_pasing2 = int(traci.simulation.getParameter("pasing_park1", "parkingArea.capacity")) - (num_vehicle_pasing2 + len(shuttle_rebalanced["pasing2"]))
